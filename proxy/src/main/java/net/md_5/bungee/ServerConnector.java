    ...
    public void handle(Login login) throws Exception
    {
        ...
        if ( user.getServer() == null )
        {
            ...
            if (login instanceof network.ycc.waterdog.pe.packet.PELogin){
                ...
                int clientId = ProxyServer.getInstance().createClientId(peLogin.getEntityId());
                user.setClientEntityId(clientId);
            }
            ...
            if (login instanceof network.ycc.waterdog.pe.packet.PELogin) {
                ...
                modLogin = new network.ycc.waterdog.pe.packet.PELogin( user.getClientEntityId(),
                ...
                // Since 419 block palette is not sent by server
                if (user.getPendingConnection().getVersion() < ProtocolConstants.MINECRAFT_PE_1_16_100){
        ...
        } else
        {
            ...
            if (login instanceof network.ycc.waterdog.pe.packet.PELogin) {
                ...
                server.unsafe().sendPacket(new network.ycc.waterdog.pe.packet.PELocalPlayerInitialized(user.getClientEntityId()));
